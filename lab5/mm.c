...
void* mm_malloc(size_t size) {
  ...
  // Find block of requested size in free list
  ptr_free_block = search_free_list(req_size);

  // If no large enough block, then make space on heap
  if(ptr_free_block == NULL){
    request_more_space(req_size);
    ptr_free_block = search_free_list(req_size);
  }

  // Remove the block from free list and save its size and whether the preceding block is used
  ...
  // If removed block is bigger than what we need, then we must split 
  if(block_size - req_size >= MIN_BLOCK_SIZE) {
    // setting tags for requested block
    ptr_free_block->size_and_tags = req_size | TAG_USED;
    ptr_free_block->size_and_tags = ptr_free_block->size_and_tags | preceding_block_use_tag;

    // updating size and tags in remaining freed block header and footer
    size_t newTags = (block_size - req_size) | TAG_PRECEDING_USED;
    size_t* headerPtr = (size_t*)UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    size_t* footerPtr = (size_t*)UNSCALED_POINTER_ADD(ptr_free_block, block_size - WORD_SIZE);
    *headerPtr = newTags;
    *footerPtr = newTags;
    block_info* remaining_free_block = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    
    insert_free_block(remaining_free_block);
  } else { // Removed block is the amount of requested memory 
    // Set next block preceding used tag
    block_info* nextBlock = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block, block_size);
    nextBlock->size_and_tags = nextBlock->size_and_tags | TAG_PRECEDING_USED;
    // Set this allocated block's used tag
...
void mm_free(void* ptr) {
  ...
  // Get pointer to the header of the block to free and it's block size
  ...
  // Set used tag in header to 0 and set the bits of the footer to the same as the header
  block_to_free->size_and_tags = block_to_free->size_and_tags ^ TAG_USED; //header
  size_t* footerPtr = (size_t*)UNSCALED_POINTER_ADD(block_to_free, payload_size - WORD_SIZE);
  *footerPtr = block_to_free->size_and_tags; //footer

  // Set the following blocks precdeing used tag to 0
  following_block->size_and_tags = following_block->size_and_tags ^ TAG_PRECEDING_USED;

  // coalesce the free block and instert it into the free list following LIFO policy
