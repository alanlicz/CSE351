...
unsigned short get_section(unsigned long* aisle, int index) {
  return *aisle >> SECTION_SIZE*index;  // discard everything else
}
...
unsigned short get_spaces(unsigned long* aisle, int index) {
  ...
  return section & SPACES_MASK; //preserve the 10 least significant bits
}
...
unsigned short get_id(unsigned long* aisle, int index) {
  unsigned short section;
  section = get_section(aisle, index);
  return (section & ID_MASK) >> NUM_SPACES; //preserve the 6 most significant bits
}
...
void set_section(unsigned long* aisle, int index, unsigned short new_section) {
  *((short*)(aisle) + index) = new_section; 
  //cast the pointer into short type so the pointer will increase by 2 bytes instead of 8
}
...
void set_id(unsigned long* aisle, int index, unsigned short new_id) {
  ...
  if (new_id >> ID_SIZE == 0) {
    section = get_section(aisle, index);
    spaces = section & SPACES_MASK;
    new_section = spaces | (new_id << NUM_SPACES);
    *((short*)(aisle) + index) = new_section;
    }
}
...
void toggle_space(unsigned long* aisle, int index, int space_index) {
  *((short*)(aisle) + index) ^= 1 << space_index;
}
...
void add_items(unsigned long* aisle, int index, int n) {
  ...
  unsigned short newSpace;
  unsigned short spaces;
  emptyCount = NUM_SPACES - num_items(aisle, index);
  if (emptyCount <= n){
    newSpace = SPACES_MASK;
    set_spaces(aisle, index, newSpace);
  } else {
    spaces = get_spaces(aisle, index);
    for (int i = 0; n > 0; i++){
      if ((spaces & 1) == 0) {
...
void remove_items(unsigned long* aisle, int index, int n) {
  unsigned short occupyCount;
  unsigned short newSpace;
  unsigned short spaces;
  occupyCount = num_items(aisle, index);
  if (occupyCount <= n){
    newSpace = 0;
    set_spaces(aisle, index, newSpace);
  } else {
...
void rotate_items_left(unsigned long* aisle, int index, int n) {
  ...
  space = get_spaces(aisle, index);
  newSpace = (space << (n % NUM_SPACES)) | (space >> (NUM_SPACES - (n % NUM_SPACES)));
  newSpace &= SPACES_MASK;
  set_spaces(aisle, index, newSpace);
}
